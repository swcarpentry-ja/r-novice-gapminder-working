...
報告書にある、_全て_ の数を再現可能なものにすることができます。 Use <code>\`r</code> and <code>\`</code> for an in-line code chunk,
like so: ` ``r "r round(some_value, 2)"`` `. 例えば、 <code \`r round(some_value, 2)\`</code 。このコードはコンパイル時に 実行され、結果の _値_ に置き換えられます。
